...
def graf_pizza(df, coluna, titulo):
    """
...
def graf_hist_desvpad(df, coluna, titulo):
    """
...
def graf_bar_par(df, coluna_x, colunas_y, title, xlabel, ylabel):
    """
    ...
    if not all([coluna_x, colunas_y, title, xlabel, ylabel]):
        raise ValueError("Todos os parâmetros são obrigatórios.")
    ...
    try:
        ...
        for i, y_column in enumerate(colunas_y):
            offset = i * bar_width
        ...
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        plt.xticks(posicoes + bar_width * (len(colunas_y) - 1) / 2, categorias)
        plt.legend()
        plt.grid(True)
        plt.show()

    except KeyError as e:
        raise KeyError(f"A coluna {str(e)} não existe no DataFrame.")


def graf_curvas(dataframe, coluna_x, coluna_y, title, xlabel, ylabel):
    """
    ...
    if not all([dataframe, coluna_x, coluna_y, title, xlabel, ylabel]):
        raise ValueError("Todos os parâmetros são obrigatórios.")
    ...
    try:
        ...
        for y_column in coluna_y:
            plt.plot(dataframe[coluna_x], dataframe[y_column], label=f'{y_column}')
